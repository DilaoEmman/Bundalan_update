...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Fetch random active farewell message
            $farewell = \App\Models\FarewellMessage::where('active', 1)->inRandomOrder()->first();
            $farewellMessage = $farewell ? $farewell->message : null;

            // Prepare response with low stock warnings if any
            $response = [
                'order' => $data['order'],
                'farewell_message' => $farewellMessage, // <-- Add this line
            ];
